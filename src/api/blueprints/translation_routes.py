...
def create_translation_blueprint(state_manager, start_translation_job):
    ...
    def resume_translation_job_endpoint(translation_id):
        ...
        if len(active_translations) >= 5:
            active_info = ', '.join([f"{t['output_filename']} ({t['status']})" for t in active_translations])
            return jsonify({
                "error": "Cannot resume: max concurrent translations reached",
                "message": f"Maximum of 5 concurrent translations reached. Please wait for one to complete or interrupt it. Active: {active_info}",
